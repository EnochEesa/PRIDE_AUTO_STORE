...
export default function ProductCard({ product }: ProductCardProps) {
  ...
  const fallback =
    FALLBACK_IMAGES[parseInt(product._id) % FALLBACK_IMAGES.length] ||
    FALLBACK_IMAGES[0];
      ...
      <div className="relative overflow-hidden aspect-square bg-dark-700">
        ...
        {product.category && (
          <div className="absolute top-3 left-3">
            <span className="bg-dark-900/80 backdrop-blur-sm text-white/60 text-[10px] font-semibold tracking-widest uppercase px-2 py-1">
              {product.category}
            </span>
          </div>
        )}
        ...
        {isLowStock && !isOutOfStock && (
          <div className="absolute top-3 right-3">
            <span className="bg-amber-500/90 text-dark-900 text-[10px] font-bold tracking-wider uppercase px-2 py-1">
              Low Stock
            </span>
          </div>
        )}
        {isOutOfStock && (
          <div className="absolute top-3 right-3">
            <span className="bg-red-500/90 text-white text-[10px] font-bold tracking-wider uppercase px-2 py-1">
              Out of Stock
            </span>
          </div>
        )}
        ...
        <div className="absolute inset-0 bg-dark-900/60 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
          <button className="bg-white/10 backdrop-blur-sm border border-white/20 text-white p-3 hover:bg-brand-500 hover:border-brand-500 transition-all duration-200">
      ...
      <div className="p-4 flex flex-col flex-1">
        ...
        <div className="flex gap-0.5 mb-2">
          {Array.from({ length: 5 }).map((_, i) => (
            <Star
              key={i}
              className={`w-3 h-3 ${i < 4 ? "text-brand-400 fill-brand-400" : "text-white/20"}`}
            />
          ))}
        </div>

        <h3 className="text-white font-semibold text-sm leading-snug mb-1 line-clamp-2 flex-1">
          {product.name}
        </h3>
        ...
        <div className="flex items-center justify-between mt-auto pt-3 border-t border-white/5">
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-2 text-xs font-semibold tracking-wider uppercase transition-all duration-200 ${
              isOutOfStock
